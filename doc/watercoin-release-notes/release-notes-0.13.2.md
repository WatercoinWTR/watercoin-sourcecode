WaterCoin Core version 0.13.2 is now available from:

  <https://download.watercoin.org/watercoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/watercoin-project/watercoin/issues>
...
updates will be released anymore. Without security updates, using a watercoin
...
ECDSA signatures inside WaterCoin transactions now use validation using
...
Previous versions of WaterCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of WaterCoin Core was to raise the default minimum
relay fee.

WaterCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
WaterCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running watercoind without having to do any manual configuration.
...
WaterCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. WaterCoin Core will relay transactions with
...
entirely in WaterCoin Core 0.13, and it is at this time undecided whether the
...
WaterCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
WaterCoin Core automatically creates a hidden service to listen on, without
manual configuration. WaterCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if WaterCoin Core is listening, and
...
WaterCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals WaterCoin
Core to use floating fees. By default, WaterCoin Core will use floating
fees.
...
(default: `0.0002` WTR/kB).

At all times, WaterCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) WTR.
Furthermore, WaterCoin Core will never create transactions paying less than
...
`watercoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 212669 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 212669 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /watercoinrpc>
        ProxyPass http://127.0.0.1:12669/
        ProxyPassReverse http://127.0.0.1:12669/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass watercoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< watercoinrpc:<password>
    </Location>
...
Restarting watercoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in watercoin.conf
...
watercoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/watercoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The WaterCoin Core code base has
...
- `watercoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `watercoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the WaterCoin scripting system that in combination with
...
WaterCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of WaterCoin Core.
...
The two phases are distinct in the WaterCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming WaterCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for WaterCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each WaterCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of watercoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of watercoins being spent is exactly
...
- **Increased security for multisig:** WaterCoin addresses (both P2PKH addresses
  ...
  believed to be provided by WaterCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of WaterCoin's current ledger (called the UTXO set),
  ...
  note that WaterCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  WaterCoin users to individually opt-in to almost any change in the WaterCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and WaterCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the WaterCoin
client to WaterCoin Core 0.13.2 to signal for segwit support. When a
...
Since WaterCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `watercoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `watercoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
